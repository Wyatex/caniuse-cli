...
const IGNORED_INSTANCE_METHODS = new Set([
  ...
  'split', 'replace', 'substring', 'substr', 'charAt', 'charCodeAt',
  'toLowerCase', 'toUpperCase', 'trim', 'search', 'match',

  // Object / Function / Symbol 的基础方法
  'toString', 'hasOwnProperty', 'isPrototypeOf', 'propertyIsEnumerable', 'valueOf',
  'bind', 'call', 'apply',
...
function initializeDynamicMappings() {
  ...
  for (const feature of Object.keys(coreJsData)) {
    if (!feature.startsWith('es.') && !feature.startsWith('esnext.')) continue;
    ...
    if (parts.length === 2) {
      ...
      // 注册静态方法
      if (!DYNAMIC_STATIC_METHODS[objectName]) DYNAMIC_STATIC_METHODS[objectName] = {};
